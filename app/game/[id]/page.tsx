...
export default function GameDetailPage() {
  ...
  const timezoneName = gameTime.toLocaleTimeString('en-US', { timeZoneName: 'short' }).split(' ').pop()
      ...
      <header className="border-b border-orange-900/20 bg-slate-950/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <Link href="/" className="flex items-center gap-3 hover:opacity-80 transition">
            <img src="/icon-only.png" alt="EdgeRusher" className="h-12 w-auto" />
          </Link>
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl border border-slate-700/50 p-8 mb-6">
          ...
          <p className="text-slate-400 text-lg mb-6">
            {gameTime.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })} at {gameTime.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })} {timezoneName}
          </p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="w-full bg-slate-800 rounded-full h-3">
                  <div className="bg-gradient-to-r from-orange-500 to-yellow-500 h-3 rounded-full transition-all" style={{ width: `${game.team_strength.home_offense}%` }} />
                </div>
              </div>
              <div>
                <div className="flex justify-between text-sm mb-2">
                ...
                <div className="w-full bg-slate-800 rounded-full h-3">
                  <div className="bg-gradient-to-r from-blue-500 to-cyan-500 h-3 rounded-full transition-all" style={{ width: `${game.team_strength.home_defense}%` }} />
                </div>
              </div>
            </div>
          </div>
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="w-full bg-slate-800 rounded-full h-3">
                  <div className="bg-gradient-to-r from-orange-500 to-yellow-500 h-3 rounded-full transition-all" style={{ width: `${game.team_strength.away_offense}%` }} />
                </div>
              </div>
              <div>
                <div className="flex justify-between text-sm mb-2">
                ...
                <div className="w-full bg-slate-800 rounded-full h-3">
                  <div className="bg-gradient-to-r from-blue-500 to-cyan-500 h-3 rounded-full transition-all" style={{ width: `${game.team_strength.away_defense}%` }} />
                </div>
              </div>
            </div>
          </div>
